...
if [ -f "$DISTROROOT/etc/nslcd.conf" ]; then
    if [ -n "$EXTERNAL_LDAP_CONFIG" ]; then
        ...
        if [ -d "$DISTROROOT/etc/ssl/certs" ] && [ -f "/etc/ssl/certs/efserver.pem" ]; then
            cp -a /etc/ssl/certs/efserver.pem "$DISTROROOT/etc/ssl/certs"
            ...
            sed -i "s#/etc/ssl/certs/ca-certificates.crt#/etc/ssl/certs/efserver.pem#g" "$DISTROROOT/etc/nslcd.conf"
        fi
    fi
fi
